...
const handleSubmit = async (recipe, {resetForm} ) => {
    ...
    for (let i = 0; i < imagePath.length; i++) {
        ...
        async function storeImage(fileName) {
            const base64 = await fs.readFile(fileName.uri, 'base64');
            console.log(base64)
            // const store = await Storage.put(fileName, recipeImageStack);
            // const blob = await fileName.blob()
            // console.log(blob)
            const store = Storage.put(recipe.name + 'Image' + i + '.jpeg', fileName, {
                ContentType: 'base64',
                key: recipe.name + 'Image' + i + '.png'

            })
            //const product = {fileName: fileName };
        ...
        .then((result) => {
            ...
            console.log(objectUrl, 'stored image')
            return imageArray.push(objectUrl)
        })
