...
const handleSubmit = async (recipe, {resetForm} ) => {
    try {
        console.log(recipe)
    // let blobData = new Blob([new Uint8Array(recipe.images[0])], {type: 'image/png'})
    // const result = await fetch(blobData)
        const resImage = await fetch(recipe.images[0])
        console.log('ran fetch successfully');
        const blob = await resImage.blob()
        console.log('created blob')

        console.log(resImage);
        // const { key } = await Storage.put(recipe.name + 'Image' + '.png', blob, {
        //     contentType: 'image/png',
        //     ACL: 'public-read'
        // })
        await Storage.put(recipe.name + 'Image' + '.png', blob, {
            contentType: 'image/png',
            ACL: 'public-read'
        })
        ...
        // console.log(key)
        console.log('reaching end of try');
